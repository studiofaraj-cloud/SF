...
export const LogoLoop = React.memo<LogoLoopProps>(
  ({
    ...
    // Reduce speed on mobile for better performance, but ensure it's still visible
    // Minimum speed of 80px/s on mobile to ensure smooth visible scrolling
    const effectiveSpeed = isMobile ? Math.max(80, speed * 0.7) : speed;

    const targetVelocity = useMemo(() => {
      const magnitude = Math.abs(effectiveSpeed);
      ...
      const speedMultiplier = effectiveSpeed < 0 ? -1 : 1;
      return magnitude * directionMultiplier * speedMultiplier;
    }, [effectiveSpeed, direction, isVertical]);
    ...
    const handleMouseEnter = useCallback(() => {
      ...
      if (effectiveHoverSpeed !== undefined && !isMobile) setIsHovered(true);
    }, [effectiveHoverSpeed, isMobile]);
    const handleMouseLeave = useCallback(() => {
      if (effectiveHoverSpeed !== undefined && !isMobile) setIsHovered(false);
    }, [effectiveHoverSpeed, isMobile]);
