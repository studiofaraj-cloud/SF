  ...
  const features = [
    {
      ...
      title: t('features.customStore.title'),
      description: t('features.customStore.description')
    },
    {
    ...
    <div className="bg-background text-foreground overflow-x-hidden">
      ...
      <section className="relative min-h-[80svh] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 z-10 pointer-events-none overflow-hidden">
          <div className="floating-shape absolute top-[20%] left-[10%] w-20 h-20 md:w-32 md:h-32 border-2 border-emerald-500/30 rotate-45 hidden sm:block" />
          <div className="floating-shape absolute top-[60%] right-[15%] w-16 h-16 md:w-24 md:h-24 border-2 border-green-500/20 rounded-full" style={{ animationDelay: '2s' }} />
        </div>
        ...
        <div className="relative z-20 container px-4 sm:px-6 md:px-8 text-center">
          <div className={`transition-all duration-1000 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <p className="text-base sm:text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto mb-8 leading-relaxed">
              {t('subtitle')} <span className="text-emerald-400 font-semibold">{t('subtitleHighlight')}</span>{t('subtitleEnd')}
            </p>
        ...
        <div className="container relative z-10 px-4 sm:px-6 md:px-8">
          <ScrollFadeIn animation="fade-up">
            <div className="text-center max-w-3xl mx-auto mb-16">
              <Badge className="badge-futuristic mb-4 bg-emerald-500/20 text-emerald-400 border-emerald-500/30">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, index) => (
              <ScrollFadeIn key={index} animation="fade-up" delay={index * 100}>
                <Card className="h-full holographic-card neon-border bg-card/80 backdrop-blur-sm transition-all duration-500 hover:shadow-xl hover:shadow-emerald-500/20 group">
                  <CardContent className="p-6">
                    <div className="w-14 h-14 rounded-xl bg-emerald-500/20 flex items-center justify-center text-emerald-400 mb-4 group-hover:bg-emerald-500 group-hover:text-white transition-all">
                      {feature.icon}
                    </div>
                ...
                {platforms.map((platform, index) => (
                  <Card key={index} className="holographic-card neon-border">
                    <CardContent className="p-4 flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-bold text-emerald-400">{platform.name}</h4>
            ...
            <ScrollFadeIn animation="fade-left">
              <Card className="holographic-card neon-border overflow-hidden">
                <CardContent className="p-8">
                  <h3 className="text-2xl font-bold mb-6 text-emerald-400">{t('benefits.title')} {t('benefits.titleHighlight')}</h3>
      ...
      {/* CTA Section */}
      <section className="relative py-20 md:py-32 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/20 via-background to-green-500/10" />
        <div className="absolute inset-0">
          <div className="absolute top-1/4 left-1/4 w-64 h-64 md:w-96 md:h-96 bg-emerald-500/20 rounded-full blur-[120px] animate-pulse" />
        </div>
